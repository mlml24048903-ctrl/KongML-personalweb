const STORAGE={outerItems:"km-portfolio-outer-items-v3",connections:"km-portfolio-connections-v3",innerItems:"km-portfolio-inner-items-v3",desktopMode:"km-portfolio-desktop-mode-v1"};
["km-portfolio-outer-items-v2","km-portfolio-connections-v2","km-portfolio-inner-items-v2"].forEach(key=>localStorage.removeItem(key));
const defaultOuterItems=[
{id:"note-edge",color:"orange",title:"KEEP THE EDGE",content:"对世界保持敏感，对答案保持怀疑。",x:.205,y:.2,rotation:-3},
{id:"note-unfinished",color:"bone",title:"STAY UNFINISHED",content:"完成不是终点，而是下一次生长的起点。",x:.75,y:.56,rotation:2},
{id:"note-next",color:"lime",title:"NEXT / AI × PRODUCT × CULTURE",content:"在语言、文化与技术的交界处，寻找新的体验。",x:.14,y:.6,rotation:-1}
];
const defaultConnections=[{id:"line-1",from:"note-edge",to:"note-next"},{id:"line-2",from:"note-next",to:"note-unfinished"}];
...
function printMarkdownDocument(item){
  ...
  paper.className="printed-document throwable-paper is-printing-document";if(item._longDocument)paper.classList.add("is-long-document");paper.tabIndex=0;paper.dataset.rotation=rotation;paper.dataset.paperRadius="32";stampPrintOrder(paper);paper.style.setProperty("--document-rotation",`${rotation}deg`);paper.style.setProperty("--card-rotation",`${rotation}deg`);paper.style.transformOrigin="50% 50%";const pageNumber=item._pageNumber||1,pageTotal=item._pageTotal||1,meta=item._longDocument?"":`<span class="printed-document__meta">MARKDOWN / LOCAL PRINT</span>`;paper.innerHTML=`${meta}<h2>${esc(item.title)}</h2><div class="printed-document__content">${markdownToHTML(item.content)}</div><small>PAGE ${String(pageNumber).padStart(2,"0")} / ${String(pageTotal).padStart(2,"0")}</small>`;layer.append(paper);bindBusinessCard(paper);
