...
function loadInnerItems(){
  ...
  const fixed=defaultInnerItems.map(item=>{const current=saved.find(entry=>entry.id===item.id);return current?{...current,kind:item.kind,title:item.title,content:item.content,parentId:item.parentId}:clone(item)});
  return [...fixed,...saved.filter(item=>!defaults.has(item.id))];
}
